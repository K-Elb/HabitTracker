...
struct ColorChooser: View {
    ...
    var shadedRGB: (Int,Int,Int) {
        if let (r,g,b) = parseRGB(selectedColor) {
    ...
    var body: some View {
        VStack {
            ...
            VStack {
                ...
                Slider(value: $shade, in: -0.15...0.15)
                    .tint(.wb)
            }
            .padding()
            .background(Color(red: Double(shadedRGB.0) / 255, green: Double(shadedRGB.1) / 255, blue: Double(shadedRGB.2) / 255), in: RoundedRectangle(cornerRadius: 16))
            .padding(.top, 8)
        }
    }
    
    func button(_ color: String) -> some View {
        Button(action: { selectedColor = color; shade = 0 }) {
            Circle()
